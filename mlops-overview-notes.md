...
| **Ví dụ InsightHub** | Model embed train trên PDF tiếng Anh, user bỗng upload nhiều tài liệu tiếng Việt → vector chất lượng kém | Embedding space thay đổi → retrieval chính xác thấp hơn |
...
Rollback có **2 tầng** — DevOps làm cả hai:

```bash
# Tầng serving (K8s): quay về image/model version trước
kubectl set image deployment/model-serving model=previous_version

# Tầng registry (MLflow): hạ model lỗi, promote lại bản tốt trước đó
mlflow models transition --run-id <new_bad>  --stage Archived
mlflow models transition --run-id <prev_good> --stage Production
```
