...
```
baseline   = avg_over_time(current[30m])      # giá trị "bình thường" của 30 phút
upper_band = baseline + 3 * stddev_over_time(current[30m])
fire khi:  current > upper_band  (hoặc > ngưỡng tuyệt đối, fallback cho lab)
```

- `current` (vd `avg5m`): giá trị hiện tại đã làm mượt.
- `baseline`: **trung bình cửa sổ DÀI (30m)** — phải KHÁC `current`.
- `3σ`: ~99.7% data trong baseline → fire khi thực sự bất thường.

> ⚠️ **Pitfall sống còn:** baseline KHÔNG được lấy chính `current` làm gốc. Nếu viết
> `upper_band = avg5m + 3σ` rồi so `avg5m > upper_band` ⇒ rút gọn `0 > 3σ` ⇒ **không bao giờ fire**.
> Baseline phải là `avg_over_time(avg5m[30m])`. Xem `job:insighthub_llm_latency_upper_band`.

Nhược điểm: **cần baseline ≥ 30 phút** trước khi alert có ý nghĩa. Lab: 5-10 phút → fallback ngưỡng tuyệt đối (vd LLM avg5m > 5s) đảm bảo alert vẫn fire khi inject incident.
...
[ ] Grafana dashboard URL → 14 panels (RED+USE+cost+anomaly), no "No data"
